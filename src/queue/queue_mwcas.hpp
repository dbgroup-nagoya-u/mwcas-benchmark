...
class QueueMwCAS
{
 ...
 public:
      ...
      -> std::optional<T>
  {
    ...
    auto *front = front_.load(std::memory_order_acquire);
    while (true) {
      auto *new_front = MwCASDescriptor::Read<Node *>(&(front->next));
      if (new_front == nullptr) return std::nullopt;
      std::atomic_thread_fence(std::memory_order_acquire);

      const auto success = front_.compare_exchange_weak(  //
          front, new_front, std::memory_order_release, std::memory_order_acquire);
      if (success) {
